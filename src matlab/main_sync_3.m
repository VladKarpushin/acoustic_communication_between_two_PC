...
packet_tail = zeros(lenghth_packet_tail); %last packet if abs(nInfBits/period - fix(nInfBits/period)) > 0
...
if lenghth_packet_tail >0
    iB = iBB1;
    iA = ImaxSignSyncB2(n_packets) + length(SignBarkerB2);
    packet_tail(:) = EstSignal(iA:iB);
end
