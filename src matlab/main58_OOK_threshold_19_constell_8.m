...
%2016-11-06 added code for signal constellation
...
nInfBits = 1024*8*2;   %number of information bits
...
x = linspace(0,kt*nTotalBits*2*pi,nTotalBits*Samples);
...
%     iB = iA+Samples-1;
%     if signal(n) == 1
% %        u(iA:iB) = -uC0(iA:iB);   %BPSK f0
%         u(iA:iB) = uC0(iA:iB);     %OOK
%     end
% end
...
%[CorrIntegral SignalComplex] = CalcNoncoherentReception(z,Samples,F,Fs);    %SignAmpl0 - квадратура огибающей amplitude envelope quadrature
...
%[SignAmpCoherent] = CalcCoherentReception(z,Samples,F,Fs);
%figure,plot(x,SignAmp,'r',x,SignAmpCoherent,'g',x,z,'b');
%title('SignAmp r    SignAmpCoherent g   z b);
...
threshold = 0:0.01:0.6;  %resolver threshold. It is compared with SignAmpl = SignRS_smooth.^2+SignRC_smooth.^2;   %detected amplitude
n = length(threshold);
BER = (-2)*ones(n,1);
...
%    [EstSignal_b MaxSignSync(i) MinSignSync(i) Err] = CalcSignalEstimationNew(CorrIntegral,threshold(i), SignBarkerLong, Samples); %This function estimates information bits (information signal)
    [EstSignal_b MaxSignSync(i) MinSignSync(i) Err] = CalcSignalEstimationNew2OOK(CorrIntegral,threshold(i), SignBarkerLong, Samples, SignalComplex); %This function estimates information bits (information signal)
...
[EstSignal_b a b c SignalContell] = CalcSignalEstimationNew2OOK(CorrIntegral,threshold(i), SignBarkerLong, Samples, SignalComplex); %This function estimates information bits (information signal)
