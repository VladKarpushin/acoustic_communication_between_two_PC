...
signalInf_B2B1  = Short2Long(signalInf_B2B1, Samples);
...
ccfInfB1 = CalcCCF_FFT(signalInf_B2B1,SignBarkerB1Long, 0);
figure, plot(ccfInfB1);
title('ccfInfB1');

ccfInfB2 = CalcCCF_FFT(signalInf_B2B1,SignBarkerB2Long, 0);
...
if lenghth_packet_tail >0
    if n_packets >0
        packet_tail(:) = EstSignal(ImaxSignSyncB2(n_packets) + length(SignBarkerB2Long):iBB1);
    else
        packet_tail(:) = EstSignal(iAB1:iBB1);
    end
    EstSignal_b(length(EstSignal_b) - lenghth_packet_tail+1:length(EstSignal_b)) = packet_tail;
