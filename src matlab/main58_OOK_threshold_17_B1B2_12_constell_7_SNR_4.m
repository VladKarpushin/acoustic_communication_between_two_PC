...
[EstSignal_b a a Err a a SignalContell indexA indexB] = CalcSignalEstimationNew4B1B2(CorrIntegral,threshold(i), SignBarkerB1Long,SignBarkerB2Long, Samples,nInfBits,period,SignalComplex); %This function estimates information bits (information signal)


indexA = indexA-length(SignBarkerB1Long);
indexB = indexB+length(SignBarkerB1Long);
if (indexA-4 > 1) && (indexB > 1) && (indexA < length(z)) && (indexB < length(z))
...
% if (indexA-length(SignBarkerB1Long)-4 > 1) && (indexB+length(SignBarkerB1Long) > 1)
