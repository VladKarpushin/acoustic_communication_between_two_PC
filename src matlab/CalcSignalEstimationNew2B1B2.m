...
%without plots
%for BPSK only
function [EstSignal_b MaxSignSyncB1 MinSignSyncB1 Err delta StdSignSync] = CalcSignalEstimationNew2B1B2(CorrIntegral,threshold, SignBarkerB1Long,SignBarkerB2Long, Samples,nInfBits,period)
% input:
...
if n_packets >0
    ...
    if length(p) == period
        ...
        disp(['Error. Packet size if wrong) = ',num2str(length(p))]);
        Err = 1;
        return;
    end
