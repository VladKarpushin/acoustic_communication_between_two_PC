...
nInfBits = 1000*30;   %number of information bits
...
delta           = (-2)*ones(n,1);
StdSignSync     = (-2)*ones(n,1);
threshold = 0;

for i = 1:n
    [SignAmp] = CalcCoherentReceptionNew(z,Samples,F,Fs,PLL_offset_n(i));   %coherent reception
    MaxAbsSignAmp(i) = max(abs(SignAmp));
    [EstSignal_b MaxSignSync(i) MinSignSync(i) Err delta(i) StdSignSync(i)] = CalcSignalEstimationNew2(SignAmp,threshold, SignBarkerLong, Samples); %This function estimates information bits (information signal)
...
ErrSyst = nInfBits*Samples - delta;
PLL_offset_vs_BER = [PLL_offset_n' MaxAbsSignAmp MaxSignSync MinSignSync MaxSignSync-MinSignSync BER delta ErrSyst StdSignSync];
m = -2;
i = -2;
[m i] = max(MaxSignSync-MinSignSync);   %PLL criterion1
if abs(ErrSyst(i))>Samples/2      
    [m i] = min(abs(ErrSyst));          %PLL criterion2
end
...
[SignAmp] = CalcCoherentReceptionNew(z,Samples,F,Fs,PLL_offset_n(i));   %coherent reception
[EstSignal_b] = CalcSignalEstimationNew2(SignAmp,threshold, SignBarkerLong, Samples); %This function estimates information bits (information signal)
x = 1:length(z);
%x=x/Fs;
