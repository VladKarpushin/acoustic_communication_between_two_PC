...
signal_inf_bits_i = signal_inf_bits(1: n_inf_bits2);
signal_inf_bits_q = signal_inf_bits(1 + n_inf_bits2: end);
...
x = 0:F * Td:(kt * n_total_bits_i * 2 * pi) - (F * Td);
signal_long_i = short_to_long(signal_i, samples);        % BPSK
signal_long_i = shaper_filter(signal_long_i, samples, Fs);     %filtering

signal_long_q = short_to_long(signal_q, samples);        % QPSK q part
...
tt = 3 + kt * (freq_burst_size + length(sign_barker_b1) + length(sign_barker_b2) + n_inf_bits2) / F;   %common transmit time
nBits = 24;
samples = kt * Fs / F;       %!!!! number of samples per one symbol
if abs(samples - fix(samples)) > 0                  %check Freq assignment error
    disp(['Error. Freq assignment error. samples = kt*Fs/F = ', num2str(samples)]);
