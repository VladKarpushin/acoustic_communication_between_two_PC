...
[signalInf_b errmsg] = file2signal('input\main13_1.0KB.m');
if length(errmsg) ~= 0
...
uC0 = sin(x)/1;                    %carrier signal

%uC0 = sin(x)/1;                    %f0 carrier signal
...
u = uC0;
u = 0*u;        %for passive pause
for n = 1:nTotalBits
    iA = 1 + (n-1)*Samples;
    iB = iA+Samples-1;
    if signal(n) == 1
        %u(iA:iB) = - u(iA:iB);   %BPSK f0
...
[SignAmp SignPhase] = CalcNoncoherentReception(z,Samples,F,Fs);    %SignAmpl0 - квадратура огибающей amplitude envelope quadrature
...
for i = 1:n
%    [EstSignal_b MaxSignSync(i) MinSignSync(i) Err] = CalcSignalEstimationNew(-SignAmpCoherent,threshold(i), SignBarkerLong, Samples); %This function estimates information bits (information signal)
    [EstSignal_b MaxSignSync(i) MinSignSync(i) Err] = CalcSignalEstimationNew(SignAmp,threshold(i), SignBarkerLong, Samples); %This function estimates information bits (information signal)
...
[EstSignal_b] = CalcSignalEstimationNew(SignAmp,threshold(i), SignBarkerLong, Samples); %This function estimates information bits (information signal)
