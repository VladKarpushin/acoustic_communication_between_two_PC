...
% [signalInf_b errmsg] = file2signal('input\ones_8KB.m');
% if length(errmsg) ~= 0
...
period = 1024*8*1;      %packet size
nInfBits = 6422*8;   %number of information bits
...
n = fix(nInfBits/period);
tt = 15+kt*(2*length(SignBarkerB1) + n*length(SignBarkerB2) + nInfBits)/F;   %common transmit time
...
[CorrIntegral SignPhase] = CalcNoncoherentReception(z,Samples,F,Fs);    %SignAmpl0 - квадратура огибающей amplitude envelope quadrature
...
for i = 1:n
    ...
    [EstSignal_b MaxSignSync(i) MinSignSync(i) Err] = CalcSignalEstimationNew2B1B2(CorrIntegral,threshold(i), SignBarkerB1Long,SignBarkerB2Long, Samples,nInfBits,period); %This function estimates information bits (information signal)
...
[EstSignal_b] = CalcSignalEstimationNew2B1B2(CorrIntegral,threshold(i), SignBarkerB1Long,SignBarkerB2Long, Samples,nInfBits,period); %This function estimates information bits (information signal)
